    ...
    for task in task_list:
        ...
        for data_file in data_files:
            ...
            for t in range(len(traj)):
                ...
                agent_view = traj[t]['obs']['camera_front_image']
                ...
                eef_euler = [normalize_angle(angle) for angle in eef_euler]
                if eef_euler[0] < 0:
                    print(f"Negative roll angle detected: {eef_euler[0]} at timestep {t}")
                eef_quat = mat2quat(euler2mat(eef_euler))
                ...
                for i in range(num_future_frame):
                    ...
                    next_pos_px_space = convert_from_3D_to_px_space(
                        pos = traj[min(t + (i+1)*skip_len, len(traj)-1)]['obs']['eef_pos'],
                        camera_pos = CAMERA_FRONT_POS,
                        camera_quat = CAMERA_FRONT_QUAT,
                        fovy = fov_y,
                        img_width = traj[t]['obs']['camera_front_image'].shape[1],
